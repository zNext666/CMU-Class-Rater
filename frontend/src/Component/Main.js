...
class Main extends Component{
    ...
    async componentDidMount(){
        if(window.location.pathname.search('search') > 0){
            let searchCourses = axios.get('https://cmuclassrater.engineer/api/course/search?search=' + window.location.pathname.split('/')[2])
            .then(res => {
            ...
            const response = await axios.get('https://cmuclassrater.engineer/api/courses')
    ...
    async componentWillReceiveProps(nextProps){
        if(nextProps.sort == 'score'){
            const response = await axios.get('https://cmuclassrater.engineer/api/courses/raw')
            const data = await response.data
            const pages = Math.round((data[0].COUNT_ROWS)/9)
            this.setState({pages:pages})
        }
        if(nextProps.sort == 'view'){
            const response = await axios.get('https://cmuclassrater.engineer/api/courses/raw?sort=view')
            const data = await response.data
            const pages = Math.round((data.count)/9)
            this.setState({pages:pages})
        }
        if(nextProps.sort == 'credit DESC'){
            const response = await axios.get('https://cmuclassrater.engineer/api/courses/raw?sort=credit&&order=DESC')
            const data = await response.data
            const pages = Math.round((data.count)/9)
            this.setState({pages:pages})
        }
        if(nextProps.sort == 'credit ASC'){
            const response = await axios.get('https://cmuclassrater.engineer/api/courses/raw?sort=credit&&order=ASC')
            const data = await response.data
            const pages = Math.round((data.count)/9)
            this.setState({pages:pages})
        }
