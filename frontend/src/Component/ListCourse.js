...
class ListCourse extends Component{
    ...
    async componentDidMount(){
        const response = await axios.get('https://cmuclassrater.engineer/api/api/courses')
    ...
    async componentWillReceiveProps(nextProps){
        if(nextProps.sort == 'score' || nextProps.sort == 'view' || nextProps.sort == 'credit DESC' || nextProps.sort == 'credit ASC'){
            ...
            /*const response = await axios.get('https://cmuclassrater.engineer/api/api/courses/raw')
            const data = await response.data
        ...
        if(this.state.sort != ''){
            if(this.state.sort == 'score'){
                const response = await axios.get('https://cmuclassrater.engineer/api/courses/raw?page=' + nextProps.page)
                const data = await response.data
                this.setState({courses:data})
                console.log('sort data ', this.state.courses)
            }
            if(this.state.sort == 'view'){
                const response = await axios.get('https://cmuclassrater.engineer/api/courses/raw?sort=view&&page=' + nextProps.page)
                const data = await response.data
                this.setState({courses:data.rows})
                console.log('sort data ', this.state.courses)
            }
            if(this.state.sort == 'credit DESC' || this.state.sort == 'credit ASC'){
                ...
                const response = await axios.get('https://cmuclassrater.engineer/api/courses/raw?sort=credit&&order='+ order +'&&page=' + nextProps.page)
                const data = await response.data
                this.setState({courses:data.rows})
                console.log('sort data ', this.state.courses)
            }
        }else{
            const response = await axios.get('https://cmuclassrater.engineer/api/courses?page=' + nextProps.page)
